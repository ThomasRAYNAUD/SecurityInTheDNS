...
def run_dig_command(ip_address, website, algo_id, timeouts):
    command = f"dig +dnssec @{ip_address} {website} SOA | grep -E 'IN\s+SOA'" #'RRSIG\s+SOA'
    try:
        result = subprocess.run(command, shell=True, capture_output=True, timeout = 20).stdout.decode()        
        if result != "" :
            print(f"ip : {ip_address} website : {website}")
            print(result)
        ...
        print(f"Timeout ocurred while executing command for ip {ip_address} and website {website}")
        timeouts += 1
        ...
        for line in file:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
                for website in websites:
                    thread = executor.submit(run_dig_command, ip_address, website, algo_id, timeouts)
